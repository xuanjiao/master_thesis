...
% \begin{itemize}
% \item Evaluate the need for decision variables.
...
% % \item Investigate how different weight combinations in cost functions affect the performance \cite{Shiqi}. 
% \end{itemize}
...
% \paragraph{Sampling Design} When simulation started, robots ran charging task (Task 1-3) at their charging station and started charging (Figure \ref{fig:charging_station_event}). For example, robot 1 charged at charging station 1, robot 2 charged at charging station 2, robot 3 charged at charging station 3. When robot fully charged, the first experiment started (Figure \ref{fig:env_exp_timeline}). After that, the task scheduling module in the centralized pool created a ``gather information task'' for each robot requested a task. After a constant experiment duration T, experiments were finished and robots went to their charging stations. These rules ensured that 1) Each Robot always started at an initial position. 2) Robots would not shut down because of power exhaustion. 3) Robots spent the same time gathering information.
...
\subsection{ Find the Best Weight Combinations}
\paragraph{Experiment Introduction}
...
%  When robot fully charged, the first experiment started (Figure \ref{fig:navigation_task_experiment_timeline}), and 15 ``navigation tasks'' and 3 ``charging tasks'' were created (Task 4-21). Especially, in the same experiment, the task time interval was the same. In Table \ref{tab:exp_task_table} the task time interval was 65 seconds. In a different experiment, the corresponding task had the same goal positions (Task 4 and 21, Task 4 and 22). 
%  When all robot finished tasks, the experiment would be finished and robots charged at the previous charging station(Task 19-21).
%  These rules ensured that robots not only started at their initial positions but also processed the same task set and not shut down because of power exhaustion.
...
% \paragraph{Sampling design}
% When the simulation started, robots ran the charging task and moved to the corresponding charging station, and started charging (Figure \ref{fig:charging_station_event}). For example, robot 1 charged at charging station 1, robot 2 charged at charging station 2, robot 3 charged at charging station 3. 
% When robot fully charged, the first experiment started (Figure \ref{fig:navigation_task_experiment_timeline}), and 15 ``navigation tasks'' and 3 ``charging tasks'' were created (Task 4-21). Especially, in the same experiment, the task time interval was the same. In Table \ref{tab:db_task_table} the task time interval was 65 seconds. In a different experiment, the corresponding task had the same goal positions (Task 4 and 21, Task 4 and 22). 
% When all robot finished tasks, the experiment would be finished and robots charged at the previous charging station(Task 19-21).
% These rules ensured that robots not only started at their initial positions but also processed the same task set and not shut down because of power exhaustion.
...
%the need of four decision variables: energy consumption, waiting time, the product of door probability, and priority. 

% \paragraph{Sampling design}
% Table \ref{tab:db_task_table} shows ``task table'' in database. At this point, robots finished task 4-21 in experiment 1 and finished 21-39 in experiment 2. When the simulation started, 3 robots ran the charging task (Task 1-3) and moved to the corresponding charging station, and started charging (Figure \ref{fig:charging_station_event}). For example, robot 1 charged at charging station 1, robot 2 charged at charging station 2, robot 3 charged at charging station 3. 
% When robot fully charged, the first experiment started (Figure \ref{fig:navigation_task_experiment_timeline}), and 15 ``navigation tasks'' and 3 ``charging tasks'' were created (Task 4-21). Especially, in the same experiment, the task time interval was the same (Table \ref{tab:db_task_table}). In different experiments, the corresponding task had the same goal positions (Task 4 and 21, Task 4 and 22). 
% When all robot finished tasks, the experiment would be finished and robots charged at the previous charging station(Task 19-21).
% These rules ensured that robots not only started at their initial positions but also processed the same task set and not shut down because of power exhaustion.
