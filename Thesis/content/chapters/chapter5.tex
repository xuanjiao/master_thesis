...
%     \item Create and open a Gazebo world with 3 robot model and a given indoor office world model  Configure the simulation 
%     \item Run navigation stack as well as Rviz. The map created with SLAM \cite{T3SLAM} should be load to the scene of Rviz(Figure \ref{fig:exp_map}).
%     \item Start SQL server and Charging event in database \ref{fig:charging_station_event}. Starts ``charging station'' node. The charging event as well as ``charging station'' simulates real charging station that can increase robot battery level by time.
%     \item Start ROS node ``sensor simulatior'' node. This node simulates real sensors attached on doors and broadcasts door status \ref{sec:sensor_simulatior}.
%     \item Start ROS nodes ``centralized pool'' and ``robot controller''.
% \end{enumerate}
...
\section{Gather Information Task Evaluation Experiment}
...
\hline
Experiment & $W_{\mbox{battery}}$ & $W_{\mbox{waiting time}}$ & $W_{\mbox{door open possibility}}$	&  $W_{\mbox{priority}}$ & Experiment Duration &Total Task & Succeeded Task &	Expired Task & Failed Task	  \\ \hline
...
\todo{Change experiment result table to graph and write more analysis according to graph}
\paragraph{Analysis.} 
